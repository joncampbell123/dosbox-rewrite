...
IDirectDrawSurface4*            ddsurfacePrimary = NULL;
IDirectDrawSurface4*            ddsurfaceGDI = NULL;    // Microsoft is very unclear about this: You can only get the GDI surface in fullscreen/exclusive. BUT, you can create a primary surface of the screen!
IDirectDrawSurface4*            ddsurfaceBMP = NULL;
...
int init_bitmap(unsigned int w,unsigned int h,unsigned int align=32) {
    DDSURFACEDESC2 ddsurf,dispsurf;
    HRESULT hr;
...
DDSURFACEDESC2 displayModes[MAX_DISPLAY_MODES];
int displayModesCount = 0;

HRESULT WINAPI populateDisplayModesEnumCallback(LPDDSURFACEDESC2 lpDD,LPVOID lpContent) {
    ...
    switch (uMsg) {
        ...
        case WM_COMMAND:
            if (wParam >= 4000 && wParam < (WPARAM)(4000+displayModesCount)) {
                DDSURFACEDESC2 devmode = displayModes[wParam-4000];
                HRESULT hr;
...
int main(int argc,char **argv) {
    ...
    if ((hr=ddraw1->QueryInterface(IID_IDirectDraw4,(void**)(&ddraw))) != S_OK) {
        fprintf(stderr,"Unable to query IDirectDraw4 from IDirectDraw, HR=0x%08lx\n",hr);
        return 1;
    }
    ddraw1->Release();
    ddraw1 = NULL;
