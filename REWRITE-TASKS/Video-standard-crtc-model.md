...
In the scaling cases the active display given by the guest is scaled up to fit the screen. Depending on the chipset, horizontal sync may be emulated either per unscaled scanline (Intel), or typical of older chipsets, per scanline up the upscaled output (Chips & Tech).
